...
async function assumeAffiliation({ affiliationId, adminId, adminName, tenantId }) {
    const current = await affiliationRepository.getAffiliationAssignmentById(affiliationId, tenantId);
    if (!current) {
    ...
    await affiliationRepository.setAffiliationAssignee(affiliationId, adminId, tenantId);
    await affiliationRepository.addAffiliationChatMessage(
        affiliationId,
        adminId,
        `Olá, eu sou o administrador ${adminName} e assumi seu protocolo ${current.protocolo}.`,
        tenantId
    );
...
async function requestTransfer({ affiliationId, adminId, tenantId }) {
    const filiacao = await affiliationRepository.getAffiliationAssignmentById(affiliationId, tenantId);
    if (!filiacao) {
    ...
    await affiliationRepository.setTransferStatusPending(affiliationId, tenantId);
}

async function denyTransferRequest({ affiliationId, requesterId, requesterRole, tenantId }) {
    ...
    await affiliationRepository.clearTransferStatus(affiliationId, tenantId);
    await affiliationRepository.addAffiliationChatMessage(
        affiliationId,
        requesterId,
...
async function transferAffiliation({ affiliationId, targetAdminId, requesterId, requesterRole, tenantId }) {
    if (requesterRole !== 'super_admin') {
    ...
    const targetAdmin = await affiliationRepository.getAdminProfileById(targetAdminId, tenantId);
    if (!targetAdmin) {
        throw new TransferServiceError('Admin de destino não encontrado.', 404);
    }

    await affiliationRepository.transferAffiliationToAdmin(affiliationId, targetAdminId, tenantId);
    await affiliationRepository.addAffiliationChatMessage(
        affiliationId,
        requesterId,
