...
exports.getReports = async (req, res) => {
    try {
        ...
        const total = await db.get('SELECT COUNT(*) as count FROM filiacoes WHERE tenant_id = ?', [tenantId]);
        const pending = await db.get("SELECT COUNT(*) as count FROM filiacoes WHERE status IN ('em_analise', 'em_processamento') AND tenant_id = ?", [tenantId]);
        const approved = await db.get("SELECT COUNT(*) as count FROM filiacoes WHERE status = 'concluido' AND tenant_id = ?", [tenantId]);
        const rejected = await db.get("SELECT COUNT(*) as count FROM filiacoes WHERE status = 'rejeitado' AND tenant_id = ?", [tenantId]);
        const todayCount = await db.get("SELECT COUNT(*) as count FROM filiacoes WHERE date(data_solicitacao) = date('now') AND tenant_id = ?", [tenantId]);
        ...
        const approvalTimes = await db.all(`
            ...
            FROM filiacoes 
            WHERE status = 'concluido' AND data_aprovacao IS NOT NULL AND tenant_id = ?
        `, [tenantId]);
        ...
        const rejectionReasons = await db.all(`
            ...
            FROM filiacoes 
            WHERE status = 'rejeitado' AND tenant_id = ?
            GROUP BY reason 
            ORDER BY count DESC 
            LIMIT 5
        `, [tenantId]);
